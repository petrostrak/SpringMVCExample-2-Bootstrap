...
<body>
	...
	<div class="well lead">Trainer Registration Form</div>
 	<form:form method="POST" modelAttribute="trainer" class="form-horizontal">
		...
		<div class="row">
			<div class="form-actions floatRight">
				<c:choose>
					<c:when test="${trainerService.getUpdateUrl()}">
						<input type="submit" value="Update" class="btn btn-primary btn-sm"/> or <a href="<c:url value='/${trainerService.getListUrl()}' />">Cancel</a>
					</c:when>
					<c:otherwise>
						<input type="submit" value="Register" class="btn btn-primary btn-sm"/> or <a href="<c:url value='/${trainerService.getListUrl()}' />">Cancel</a>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
